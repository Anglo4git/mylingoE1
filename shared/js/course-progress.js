...
// Agent 4 (MASTERY-THRESHOLD): a quiz only counts toward "lesson complete"
// once it's been scored at MASTERY_THRESHOLD% or higher — not just
...
function lessonCompletionRecord(lesson,p){
  var finalId=lesson&&lesson.lesson_quiz_id;
  if(finalId){
    var finalRecord=p[finalId];
    if(isMastered(finalRecord)) return {completed:true,progress:100,gateId:finalId};
    if(finalRecord&&finalRecord.status==='in-progress') return {completed:false,progress:Math.max(1,sessionRatio(finalId,finalRecord.totalQuestions||0)),gateId:finalId};
    return {completed:false,progress:0,gateId:finalId};
  }
  ...
  if(done===ids.length)return {completed:true,progress:100,gateId:ids[ids.length-1]};
  ...
  return {completed:false,progress:Math.round(done/ids.length*100),gateId:null};
}
...
global.MylingoCourseProgress={readProgress:progress,lessonPercent:lessonPercent,lessonCompletionRecord:lessonCompletionRecord,lessonIsComplete:lessonIsComplete,resolveHomepageState:resolveHomepageState,esc:esc,isMastered:isMastered,MASTERY_THRESHOLD:MASTERY_THRESHOLD};
})(window);
